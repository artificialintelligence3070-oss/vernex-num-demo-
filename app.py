...
# --- RENDER SSL & PROXY HARDENING ---
# 1. Generate a consistent fall-back signing key if environment variable missing
app.secret_key = os.environ.get("FLASK_SECRET_KEY", "matrix_fallback_secure_string_9901")

# 2. Tell Flask it is behind a Render reverse proxy so sessions function over HTTPS
app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_proto=1, x_host=1, x_prefix=1)

# 3. Secure session cookie directives for deployment environments 
app.config.update(
    SESSION_COOKIE_SECURE=True,      # Only send cookies over HTTPS
    SESSION_COOKIE_HTTPONLY=True,    # Prevent cross-site scripting cookie theft
    SESSION_COOKIE_SAMESITE="Lax",   # Allow standard internal navigation tracking
)
...
class Bot:
    ...
    def run(self):
        ...
        self.send(self.chat_id, "Bot started! System active.")
...
def restore_active_bots():
    try:
        ...
        print(f"[CRITICAL] Boot recovery exception: {e}")

def keep_alive_ping():
    while True:
        time.sleep(600)  # Self-ping every 10 minutes to prevent Render tier spin-down
        if "localhost" not in RENDER_HOST:
            try: requests.get(RENDER_HOST, timeout=10)
            except: pass
...
def index():
    ...
    body { font-family: 'Courier New', Courier, monospace; background-color: var(--bg-color); color: var(--text-main); padding: 15px; margin: 0; min-height: 100vh; box-sizing: border-box; }
    ...
    <div id="auth-block" class="login-container">
        <div class="login-box">
            ...
            <div id="auth-err" class="status err" style="display:none; margin-top:15px;">AUTHENTICATION_FAILURE // ACC_DENIED</div>
        </div>
    </div>
    <div class="c" id="main-panel">
    <div class="h">
        <h1>// CENTRAL OPERATIONS //</h1>
        <p>SECURE TERMINAL MATRIX</p>
    </div>
    <div class="card">
    ...
    <div class="card">
        ...
        <div class="tools">
            ...
            <div class="tool" onclick="whites(this)" data-t="ifsc_lookup">🏦 IFSC</div>
    ...
    function attemptLogin(){
      ...
      var eBox = document.getElementById("auth-err");
      eBox.style.display = "none";
      
      // Robust absolute endpoint matching + clean status reading
      fetch("./api/login", {
        method: "POST",
      ...
      .then(res => {
        ...
        } else { eBox.style.display = "block"; }
      })
      .catch(() => { eBox.style.display = "block"; });
    }
    function tog(el){
    ...
    function all(){
      ...
      else { sel = new Set(ak); document.querySelectorAll(".tool").forEach(e=>e.add("s")); }
    }
...
def api_login():
    try:
        ...
        return jsonify({"success": False, "error": "Crash"}), 400
