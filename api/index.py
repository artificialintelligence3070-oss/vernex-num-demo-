...
# --- EMERGENCY BOOTSTRAP INNER PROTECTOR ---
# This catches syntax, library, and import errors before they can cause a Vercel 500 crash
try:
        ...
        if "all" not in config['allowed_tools']:
            ...
            for tool_category in config['allowed_tools']:
                ...
                if endpoint in FREE_TOOLS:
                    matched = True
            if not matched:
                return False, "Endpoint locked under current license scope."
        return True, config
    ...
    <body>
        ...
        {% if view == "login" %}
        <div style="max-width:400px; margin:100px auto; padding:40px; text-align:center;" class="glass">
        ...
        <div class="grid">
            ...
            <div class="card glass">
                <div>
                    ...
                    <p style="color:#8a99ad; font-size:13px; margin:10px 0;">Endpoints: {{ ", ".join(pack.tools[:3]) }}...</p>
                </div>
                <div>
    ...
    def purchase_init():
        ...
        try:
            ...
            return jsonify({"status": "failed", "message": "Failed order sync.", "details": str(e)}), 400
    ...
    def payment_verification_callback():
        client = get_razorpay_client()
        if not client: return "Driver unavailable.", 503
        ...
        if not pay_id or not order_id or not signature: return "Missing explicit arguments.", 400

        sig_check_payload = f"{order_id}|{pay_id}"
        generated_signature = hmac.new(bytes(RAZORPAY_SECRET, 'utf-8'), msg=bytes(sig_check_payload, 'utf-8'), digestmod=hashlib.sha256).hexdigest()
        
        if generated_signature != signature: return "Signature mismatch.", 400
        ...
        return f"<h1>Success! Key `{assigned_key}` active.</h1><a href='/'>Return</a>"

    @app.route('/api/<endpoint>', methods=['GET'])
    def core_api_proxy_dispatcher(endpoint):
        client_auth_key = request.args.get('key')
        if not client_auth_key: return jsonify({"status": "failed", "message": "Missing key."}), 400
            
        is_valid, configuration_metadata = validate_token_access(client_auth_key, endpoint)
        if not is_valid: return jsonify({"status": "failed", "message": configuration_metadata}), 403

        cleaned_forwarding_args = request.args.to_dict()
        if 'key' in cleaned_forwarding_args: del cleaned_forwarding_args['key']
        ...
        try:
            ...
            return jsonify({"status": "failed", "message": "Upstream handshake failure."}), 504

except Exception as critical_bootstrap_error:
    # EMERGENCY FALLBACK: If anything crashes on runtime init, catch it and force a visible web output
    class FailApp:
        def __call__(self, environ, start_response):
            start_response('200 OK', [('Content-Type', 'text/plain')])
            error_details = f"BOOTSTRAP CRASH ENCOUNTERED:\n{traceback.format_exc()}"
            return [error_details.encode('utf-8')]
    app = FailApp()
