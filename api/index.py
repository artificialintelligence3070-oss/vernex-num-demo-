...
class APIKeyConfig(BaseModel):
    ...
    current_used: int = 0
    is_active: bool = True
    allowed_tools: List[str] = [] # "all" or specific endpoint names

# Mock Database (In-Memory for Serverless Demo)
# NOTE: For persistent production usage on Vercel, replace this dictionary 
# reading/writing with a free cloud database like KV, MongoDB Atlas, or Supabase.
...
# --- PUBLIC INTERFACES (HTML FRONTEND RESPONSES) ---

@app.get("/", response_class=HTMLResponse)
async def public_dashboard_interface():
    """Renders the high-end dark glassmorphism 3D-feeling public lookup module layout."""
    ...
    <head>
        ...
        <style>
            ...
            body {{
                ...
                background-image: radial-gradient(circle at 50% 50%, #14142b 0%, #0a0a12 100%);
            }}
            .navbar {{
                display: flex; justify-content: space-between; align-items: center;
                padding: 20px 40px; border-bottom: 1px solid rgba(0, 240, 255, 0.2);
            ...
            .card {{
                ...
                border-radius: 16px; padding: 30px; backdrop-filter: blur(15px);
                box-shadow: 0 20px 50px rgba(0,0,0,0.5), inset 0 0 20px rgba(0, 240, 255, 0.05);
                transform: perspective(1000px) rotateX(1deg); transition: all 0.3s ease;
            }}
            ...
            label {{ display: block; margin-bottom: 8px; font-weight: bold; tracking-letter: 1px; color: #aaa; }}
            input, select {{
                width: 100%; padding: 12px; background: rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.2);
                border-radius: 8px; color: #fff; font-size: 16px; box-sizing: border-box; transition: all 0.3s;
            }}
            input:focus, select:focus {{ border-color: var(--primary); outline: none; box-shadow: 0 0 10px rgba(0, 240, 255, 0.5); }}
            button {{
                width: 100%; padding: 14px; background: linear-gradient(45deg, var(--primary), #0072ff);
                border: none; border-radius: 8px; color: #fff; font-size: 18px; font-weight: bold; cursor: pointer;
                font-family: 'Orbitron', sans-serif; letter-spacing: 2px; transition: transform 0.2s, box-shadow 0.2s;
            }}
            button:hover {{ transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0, 240, 255, 0.4); }}
            .response-box {{
                margin-top: 30px; background: #05050a; border: 1px solid rgba(255,255,255,0.05);
                border-radius: 8px; padding: 20px; display: none; max-height: 400px; overflow-y: auto;
            }}
    ...
    <body>
        ...
        <div class="container">
            <div class="card">
                ...
                <p style="color: #888;">Input an operational key assigned to your developer profile profile to interact with the lookup nodes.</p>
                ...
                <div class="form-group">
                    <label>Target Identity Parameter Value (Number/ID/User/Query)</label>
                    <input type="text" id="targetQuery" placeholder="Ex: 9876543210, example@ybl, username">
                </div>
                ...
                <div class="response-box" id="resBox">
                    ...
                    <pre id="jsonOutput">Awaiting initialization packet...</pre>
                </div>
            </div>
        </div>
        <div class="footer">
            &copy; 2026 Powered & Developed by <strong>SHAYAN_EXPLORER</strong> Core Intelligence Systems
        </div>
        ...
        <script>
            async function executeLookup() {{
                ...
                if(!key || !target) {{
                    alert('Please ensure both target parameters and access authorization tokens are supplied.');
                    return;
                }}

                resBox.style.display = "block";
                output.innerText = "Querying proxy endpoint, decoding streams...";

                try {{
                    // Determine internal mapping parameters
                    let paramName = "num";
                    ...
                    const lookupUrl = `/api/gateway/${{endpoint}}?key=${{encodeURIComponent(key)}}&${{paramName}}=${{encodeURIComponent(target)}}`;
                    const res = await fetch(lookupUrl);
                    const data = await res.json();
                    output.innerText = JSON.stringify(data, null, 4);
                }} catch(err) {{
                    output.innerText = "Error completing endpoint fetch stream pipeline: " + err.message;
                }}
            }}
        </script>
    </body>
...
async def admin_control_panel(username: str = Depends(verify_admin)):
    """Renders the secure internal command hub dashboard configuration view."""
    
    # Generate list elements of logs
    log_rows = "".join([
    ...
    # Generate interactive configuration rows for active keys
    key_rows = "".join([
        f"""<tr>
    ...
    <head>
        <meta charset="UTF-8">
        <title>SHAYAN_EXPLORER | Secure Key Provision Systems</title>
        ...
        <style>
            ...
            .panel {{ background: #1f2833; border-radius: 8px; padding: 25px; border: 1px solid rgba(255,255,255,0.05); }}
            ...
            th {{ font-family:'Orbitron'; font-size:13px; color:#45a29e; }}
            .badge {{ background: #45a29e; color:#000; padding:2px 6px; border-radius:4px; font-size:12px; font-weight:bold; }}
            .status-indicator {{ inline-block; width:10px; height:10px; border-radius:50%; display:inline-block; }}
            .status-indicator.active {{ background: #00ff66; box-shadow: 0 0 8px #00ff66; }}
            .status-indicator.disabled {{ background: #ff0055; box-shadow: 0 0 8px #ff0055; }}
            .tbl-btn {{ background:#0b0c10; color:#66fcf1; border:1px solid #66fcf1; padding:4px 8px; border-radius:4px; cursor:pointer; font-size:12px; }}
            .tbl-btn.style-toggle {{ color:#ffbf00; border-color:#ffbf00; }}
            .logs-container {{ max-height: 300px; overflow-y:auto; background:#0b0c10; border-radius:4px; padding:10px; }}
        </style>
    </head>
    <body>
        <div class="header">
            <h1>SHAYAN_EXPLORER // SYSTEM ADMINISTRATIVE KERNEL</h1>
            <div>Authenticated Identity: <strong style="color:#66fcf1;">{username}</strong></div>
        </div>
        ...
        <div class="main-layout">
            <div class="panel">
                ...
                <form action="/admin/action/generate" method="post">
                    ...
                    <input type="text" name="key" placeholder="Ex: SHAYAN-VIP-999" required>
                    <label>Client Reference Name</label>
                    <input type="text" name="name" placeholder="Ex: Premium Client Profile" required>
                    ...
                    <select name="allowed_tools">
                        ...
                        <option value="number">Only Call/Number Lookup Modules</option>
                    </select>
            ...
            <div class="panel">
                <h2>🔑 Active Key Registry Infrastructure</h2>
                <table>
                    <thead>
                        <tr><th>Key / Identity</th><th>Expiry</th><th>Requests Used</th><th>Status</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                        {key_rows}
                    </tbody>
                ...
                <div class="logs-container">
                    <table>
                        <thead>
                            <tr><th>Time</th><th>Key Client</th><th>Module</th><th>Query Metadata</th><th>Outcome Status</th></tr>
                        </thead>
                        <tbody>
                            {log_rows}
                        </tbody>
...
async def core_proxy_gateway(endpoint: str, request: Request):
    """Intercepts requests, enforces usage criteria, masks references, routes requests safely."""
    query_params = dict(request.query_params)
    client_key = query_params.get("key", "")
    
    # 1. Enforce Token Security Policies
    is_valid, msg = validate_and_update_key(client_key, endpoint)
    
    # Extract query variable target parameters dynamically
    ...
    # 2. Re-Route Stream Payload to Target Backend
    proxy_params = query_params.copy()
    proxy_params["key"] = "" # Clears your system keys if requested or inject upstream provider values
    
    target_url = f"{TARGET_BASE_URL}/{endpoint}"
    
    try:
        response = requests.get(target_url, params=proxy_params, timeout=10)
        log_transaction(client_key, endpoint, search_query, f"SUCCESS ({response.status_code})")
        
        # Scrape or modify outbound context data dynamically to scrub third party signatures
        try:
        ...
        return JSONResponse(status_code=502, content={
            "status": "fail",
            "developer": "SHAYAN_EXPLORER",
            "reason": "Upstream lookup timeout connection termination event encountered."
        })
